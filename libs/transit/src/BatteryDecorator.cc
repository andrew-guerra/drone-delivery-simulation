...
IEntity* BatteryDecorator::GetClosestChargingStation(Vector3 location) {
  ...
  for (int i = 0; i < sim_entities.size(); i++) {
    ...
    if (name.compare("charging_station") == 0) {
      double currDist =
          GetDistanceToLocation(location, sim_entities[i]->GetPosition());
...
IEntity* BatteryDecorator::GetClosestReachableChargingStation(
  ...
  for (int i = 0; i < sim_entities.size(); i++) {
    ...
    if (name.compare("charging_station") == 0) {
      ...
      if (CanReachLocation(sim_entities[i]->GetPosition()) &&
          currDist < minDist) {
        minDist = currDist;
        closest = sim_entities[i];
      }
...
bool BatteryDecorator::CanReachCharger(Vector3 location) {
  ...
  double time =
      (location.Distance(closestCharger->GetPosition())) / entity->GetSpeed();
    ...
    case toCharger:
      if (GetPosition().Distance(closestChargerPosition) < 1) {
        ...
        IStrategy* pathStrategy =
            new BeelineStrategy(GetPosition(), closestChargerPosition);
...
double BatteryDecorator::GetBattery() const { return this->battery_life; }